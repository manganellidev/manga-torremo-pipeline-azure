...
function createBranchAndPush() {
  ...
  # Copy the ADO file
  rm $REPO_APP_DIR/azure-pipelines.yml
  cp azure-sdp-pipelines.yml $REPO_APP_DIR/azure-pipelines.yml || errorExit "Failed to copy azure-sdp-pipelines.yml."

  cd "$REPO_APP_DIR" || errorExit "Failed to change directory to $REPO_APP_DIR."
